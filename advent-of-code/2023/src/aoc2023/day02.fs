...
let readGame (data: string) =
    let string_to_pick (str: string) =
        ...
        let tokens = str.Split(",", System.StringSplitOptions.TrimEntries) |> Array.toList

        assert (tokens.Length > 0)

        for token in tokens do
            let pair =
                token.Split("\x20", System.StringSplitOptions.TrimEntries) |> Array.toList
    ...
    let parts =
        data.Split([| ":" |], System.StringSplitOptions.TrimEntries) |> Array.toList

    assert (2 = parts.Length)

    // parts[0] = "Game 1"
    let game_parts =
        parts[0].Split("\x20", System.StringSplitOptions.TrimEntries) |> Array.toList
    ...
    let pick_parts =
        parts[1].Split(";", System.StringSplitOptions.TrimEntries) |> Array.toList

    game.picks <- pick_parts |> List.map (fun str -> string_to_pick str)
...
let readGames data =
    ...
    data |> List.iter (fun line -> games <- games @ [ readGame line ])
...
let SolvePart1 data =
    ...
    let check_picks game goal =
        game.picks
        |> List.forall (fun p -> p.red <= goal.red && p.green <= goal.green && p.blue <= goal.blue)

    let good_games = games |> List.filter (fun g -> check_picks g goal)

    let answer = good_games |> List.map (fun x -> x.number) |> List.sum
