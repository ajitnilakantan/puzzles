...
let is_valid_position (pattern: string) (damage: int) (position: int) : bool =
    ...
    && (position + damage = pattern.Length || pattern[position + damage] <> '#')
...
let get_valid_positions_for_damage (pattern: string) (damage: int) : int list =
    ...
    let result =
        [ 0 .. pattern.Length - 1 ]
        |> List.collect (fun position -> if is_valid_position pattern damage position then [ position ] else [])
...
let get_base_case (pattern: string) (damaged: int list) (damaged_values: list<list<int>>) =
    ...
    while current < damaged.Length do
        ...
        let end_previous =
            if current = 0 then 0 else placement[current - 1] + damaged[current - 1]
        ...
        let next =
            ...
            |> tryFindMatch (fun x ->
                if current = 0 then
                    ...
                    x > placement[current] && not (pattern[0 .. x - 1] |> Seq.contains '#')
                else
                    x > placement[current] && x > end_previous)

        if next = None && current = 0 then failwithf "Error finding base_case"

        if next = None then
            // backtrack
            placement[current] <- -1
            current <- current - 1
        elif current > 0 && pattern[end_previous .. next.Value - 1] |> Seq.contains '#' then
...
let count_all_matches (pattern: string) (damaged: int list) (check_tail: bool) : int64 * Dictionary<int, int64> =
    let rec count_matches
        ...
        let mutable positions =
            damaged_values[index] |> List.filter (fun x -> x >= placement[index])
        ...
        if index = damaged.Length - 1 then
            ...
            if check_tail then
                positions <-
                    positions
                    |> List.filter (fun x -> not (pattern[x + damaged[index] .. pattern.Length].Contains('#')))
        else
            // Check overlap with next
            positions <- positions |> List.filter (fun x -> x + damaged[index] < placement[index + 1])
            // Check for uncovered damage between this index and index+1
            positions <-
                positions
                |> List.filter (fun x -> not (pattern[x + damaged[index] .. placement[index + 1] - 1].Contains('#')))

        if index = 0 then
            // Check no uncovered damage between begining of pattern and placement.
            positions <- positions |> List.filter (fun x -> not (pattern[0 .. x - 1].Contains('#')))

        match index with
        | 0 ->
            // Update cache
            let key = placement[placement.Length - 1] + damaged[placement.Length - 1] + 1
    ...
    let damaged_values =
        damaged |> List.map (fun d -> get_valid_positions_for_damage pattern d)
...
type Tests() =
    ...
    let ``Test Find Base Case`` () =
        ...
        Assert.Throws<System.Exception>(fun () -> get_base_case pattern damaged damaged_values |> ignore)
        |> ignore
